      ...
      public void onClick(View view) {
        if (checkItemType(itemType) && checkItemPrice(itemPriceText.getText().toString())) {
          if ((Double.valueOf(itemPriceText.getText().toString()) > 0.0f)) {
            ...
            Toast.makeText(getApplicationContext(), R.string.warning_item_is_free,
                Toast.LENGTH_SHORT).show();
          }
        } else {
          Toast.makeText(getApplicationContext(), R.string.warning_input_check_failed,
              Toast.LENGTH_SHORT).show();
        }
      }
    });
